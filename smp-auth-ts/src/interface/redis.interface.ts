 ...
 * Cache, sessions et journalisation essentiels
 */
...
// Export CacheOptions depuis common si ce n'est pas déjà fait
...
export interface RedisClientExtended extends RedisClient {
  // ============================================================================
  // CONNEXION ET ÉTAT
  // ============================================================================
  ...
  // Activité de session
  logSessionActivity(activity: Omit<SessionActivity, 'timestamp'>): Promise<void>;
  getSessionActivity(sessionId: SessionId, limit?: number): Promise<SessionActivity[]>;
  ...
  batchSet(entries: Array<{ key: string; value: string; options?: RedisOperationOptions }>): Promise<void>;
  ...
  analyzeKeyUsage(pattern?: string): Promise<Record<string, any>>;
  generateReport(type: 'usage' | 'performance' | 'sessions' | 'logs'): Promise<any>;
...
export interface RedisStats extends PerformanceMetrics {
  server: {
    version: string;
    uptime: number;
...
export const REDIS_CONSTANTS = {
  ...
  DEFAULT_LOG_TTL: 2592000,   // 30 jours
  MAX_KEY_LENGTH: 512,
